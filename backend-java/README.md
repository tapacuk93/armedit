...
The sue backend: Java 25 on Netty, matching the stack aicoin-proxy runs.

## Building

The server is Netty, so unlike the rest of sue this half has a dependency
...
| `SUE_PUBLIC_ADDR` | the address baked into issued keys |
| `SUE_AICOIN` | aicoin proxy base URL |
| `SUE_PROVIDER` | default provider aicoin routes to |
| `SUE_MODEL` | default model override |
...
| `SUE_WORKSPACE` | where accounts' folders live |
| `SUE_S3_BUCKET` | third tier of persistence |
| `SUE_IDLE_MINUTES` | terminate an idle account's instance (0 disables) |
| `SUE_REAP_SECONDS` | how often to sweep |

## Shape

| file | what it does |
|---|---|
| `SueServer.java` | Netty transport and routing; every route on a virtual thread |
