...
sue reaches models through aicoin, and aicoin multiplexes many users onto
...
its own provider key so the client never has one; sue injects the account's
...
- **The shared upstream key is still shared.** sue can keep secrets out of
